...
$db = new microDb('localhost', 'root', 'microcms', '', '');
...
class microDb
{
    ...
    private function generateOrther($order)
    {
        $str = '';
        $i = 0;
        foreach ($order as $type => $col) {
            $str .= $col . ' ' . $type;
            if ($i < count($order) - 1) {
                $str .= ', ';
            }
    ...
    public function selectByOrther($table, $elements, $whereValue, $operator, $order)
    {
        $this->generateSelectOrther($table, $elements, $whereValue, $operator, $order);
        return $this->excecuteSql($whereValue);
    }
    ...
    /**
     * Génère une requête SQL pour selectionner des lignes et les trier par ordre choisis.
     */
    private function generateSelectOrther($table, $elements, $whereValue, $operator, $order)
    {
        $this->generateSelect($table, $elements, $whereValue, $operator);
        $this->sql .= ' ORDER BY ' . $this->generateOrther($order);
    }
}
